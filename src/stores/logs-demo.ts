...
const logger = new Logger('logs-demo.store')
...
import logsDemoData from './logs-demo.json' // Import the JSON data
// get the first LIMIT logs from the JSON data
const realtimeLogs = logsDemoData.data as Log[]
const initialLogs = realtimeLogs.slice(LIMIT)
let nextLogDirection = -1
let nextLogIndex = LIMIT - 1

export const useLogsDemoStore: StoreDefinition<'logsDemo', State, Getters, Actions> = defineStore(
  'logsDemo',
  {
    state: (): State => ({
      logs: [],
    ...
    actions: {
      ...
      _updateCardDataItem(
        ...
        const digits =
          value >= 0 ? (value < 1000 ? 2 : 1) : value > -100 ? 2 : value > -1000 ? 1 : 0
      ...
      _updateChartDataItem(header: string, value: number, unit: string) {
      ...
      _processRegister(rtu: Log['reads'][number], reg: Log['reads'][number]['reads'][number]) {
        ...
        // Update Chart Data
        this._updateChartDataItem(header, numericValue, unit)
      },
      ...
      setupRealtimeUpdates() {
        try {
          ...
          this.total = logsDemoData.total

          // Simulate real-time updates
          setInterval(() => {
            const log = realtimeLogs[nextLogIndex]
            // Check if log is defined before using it
            if (log !== undefined) {
              this.addLog(log)
              this.addTotal(1)
              nextLogIndex += nextLogDirection
              if (nextLogIndex >= realtimeLogs.length || nextLogIndex < 0) {
                nextLogDirection *= -1
                nextLogIndex += nextLogDirection * 2
              }
            }
          }, 1000) // Adjust interval as needed

          // Keep the real-time update listener
