<template>
  <q-layout view="hhh lpr fff">
    ...
    <q-footer class="bg-white">
      <q-toolbar class="text-zinc-700">
        <q-item-section>
          <q-item-label
            lines="1"
            class="text-sm font-semibold max-sm:tracking-normal sm:tracking-[0.25px]"
          >
            Made with <span class="heart">❤</span> by Han Lin (hotdogee)
          </q-item-label>
        </q-item-section>
        <q-space />
        <div class="max-sm:pr-2 sm:pr-3">© {{ currentYear }}</div>
        <div class="flex flex-row max-sm:space-x-2 sm:space-x-3">
          <div
            class="h-6 w-6 fill-current hover:text-green-700 hover:drop-shadow-md dark:hover:text-green-300"
          >
            <a
