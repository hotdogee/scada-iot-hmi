...
function addLog (state, log) {
  ...
  let m1e = null
  if (m1bar && m1c) {
    m1e = (new IAPWS97_EoS()).solve({ p: m1bar.slice(-1)[0].value * 100000, t: 273.15 + m1c.slice(-1)[0].value })
  }
  let m5e = null
  if (m5bar && m5c) {
    m5e = (new IAPWS97_EoS()).solve({ p: m5bar.slice(-1)[0].value * 100000, t: 273.15 + m5c.slice(-1)[0].value })
  }
  let m6e = null
  if (m6bar && m6c) {
    m6e = (new IAPWS97_EoS()).solve({ p: m6bar.slice(-1)[0].value * 100000, t: 273.15 + m6c.slice(-1)[0].value })
  }
  ...
  const formulas = {
    '出噴嘴速度': {
      unit: 'm/s',
      value: () => {
        if (m73VA && m25tph && m71hz && m73VA.slice(-1)[0].value > 0 && m25tph.slice(-1)[0].value > 0 && m71hz.slice(-1)[0].value > 0) {
